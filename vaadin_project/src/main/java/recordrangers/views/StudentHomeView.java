...
@Route("student-home")
public class StudentHomeView extends AppLayout{
	User loggedInUser;
    public StudentHomeView() {
    	loggedInUser = (User)VaadinSession.getCurrent().getAttribute("loggedInUser");
    	if (loggedInUser == null || loggedInUser.getUserType() != User.UserType.STUDENT) {
    	    // if not signed in, or wrong user type, redirect to login screen
        ...
        setContent(new Span("Welcome to the Student Home Page"));
    }
    ...
    private void createHeader() {
        ...
        Span userInfo = new Span("Welcome " + loggedInUser.getFirstName() + "!");
    ...
    public void createDrawer() {
        ...
        Tabs tabs = new Tabs(
            new Tab(VaadinIcon.CLIPBOARD_USER.create(), new RouterLink("Profile", ProfileView.class)),
            new Tab(VaadinIcon.ACADEMY_CAP.create(), new RouterLink("Search for Courses", CourseSearchView.class)),
            new Tab(VaadinIcon.PLUS_CIRCLE_O.create(), new RouterLink("Register in a Course", CourseRegistrationView.class)),
            new Tab(VaadinIcon.PLUS_CIRCLE.create(), new RouterLink("Currentlly Enrolled Courses", CurrentCoursesView.class))
        );
